...
/// Confine del motore STT: il coordinatore non dipende dall'implementazione Apple.
/// `SpeechAnalyzer` è il provider locale predefinito su macOS 26.
...
enum TranscriptionError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .localeUnsupported(let locale):
            return "La dettatura in \(locale.identifier) non è disponibile su questo Mac."
        case .modelInstallFailed(let detail):
            return "Impossibile installare il modello vocale: \(detail)"
        case .noAudioFormat:
