...
struct DictionaryPanel: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 0) {
                SearchField(text: $query, placeholder: "Cerca nel dizionario")
                addButton
            ...
            if entries.isEmpty {
                EmptyPanel(
                    ...
                    detail: store.entries.isEmpty
                        ? "Aggiungi parole che vengono riconosciute male."
    ...
    private var addButton: some View {
        Button { isAdding = true } label: {
            HStack(spacing: DS.Space.tight) {
                ...
                Silkscreen(text: "Aggiungi", color: DS.Color.inkOnDeck)
            }
...
private struct DictionaryRow: View {
    ...
    var body: some View {
        HStack(spacing: DS.Space.base) {
            ...
            Silkscreen(
                text: entry.kind == .correction ? "Correggi" : "Termine",
            ...
            if isHovering {
                rowButton("Modifica", action: onEdit)
                rowButton(entry.isEnabled ? "Disattiva" : "Attiva", action: onToggle)
                rowButton("Elimina", action: onDelete)
            }
        }
...
private struct DictionaryEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DS.Space.roomy) {
            Silkscreen(text: entry == nil ? "Nuova voce" : "Modifica voce", large: true)
            ...
            VStack(alignment: .leading, spacing: DS.Space.base) {
                ...
                field(
                    ...
                    text: $write,
                    prompt: kind == .correction ? "WisperClone" : "WisperClone"
                )
            }
    ...
    private var kindPicker: some View {
        HStack(spacing: DS.Space.snug) {
            ForEach([DictionaryEntry.Kind.term, .correction], id: \.self) { candidate in
                TransportKey(
                    title: candidate == .term ? "Termine" : "Correzione",
