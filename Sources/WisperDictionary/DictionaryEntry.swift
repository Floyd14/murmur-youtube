...
public struct DictionaryWarning: Identifiable, Sendable {
    ...
    /// Parole comuni italiane e inglesi che attiverebbero la regola troppo spesso.
    private static let common: Set<String> = [
        ...
        "your", "il", "lo", "la", "i", "gli", "le", "un", "uno", "una", "e", "o", "ma",
        "che", "chi", "come", "cosa", "dove", "quando", "perché", "di", "da", "in", "con",
        "su", "per", "tra", "fra", "è", "sono", "essere", "avere", "ho", "hai", "ha", "abbiamo",
        "non", "sì", "no", "io", "tu", "lui", "lei", "noi", "voi", "loro", "questo", "quello",
        "fare", "dire", "andare", "vedere", "bene", "ora", "oggi", "domani", "testo", "riga",
    ]
    ...
    public static func check(_ entry: DictionaryEntry) -> [DictionaryWarning] {
        ...
        if words.count == 1, let only = words.first {
            if common.contains(String(only)) {
                warnings.append(DictionaryWarning(
                    message: "“\(trigger)” è una parola comune e verrà sostituita ogni volta. "
                        + "Valuta una frase più specifica."
                ))
            } else if only.count <= 3 {
                warnings.append(DictionaryWarning(
                    message: "“\(trigger)” è molto breve e potrebbe attivarsi spesso. Usa una frase più lunga."
                ))
            }
        }
        ...
        if entry.write.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            warnings.append(DictionaryWarning(
                message: "La regola sostituisce “\(trigger)” con sé stessa e non produrrà cambiamenti."
            ))
        }
